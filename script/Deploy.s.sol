...
contract Deploy is Script {
    ...
    function run() external {
        ...
        TribeLeaderTerminal leaderTerminal = new TribeLeaderTerminal(
            address(vaultFactory), address(leaderRegistry), UNISWAP_V3_POSITION_MANAGER, AERODROME_ROUTER
        );
        ...
        TribePerformanceTracker performanceTracker =
            new TribePerformanceTracker(address(vaultFactory), address(leaderRegistry));
        ...
        TribeLeaderboard leaderboard =
            new TribeLeaderboard(address(leaderRegistry), address(performanceTracker), address(vaultFactory));
        ...
        string memory deploymentInfo = string(
            abi.encodePacked(
                "LEADER_REGISTRY=",
                vm.toString(address(leaderRegistry)),
                "\n",
                "VAULT_FACTORY=",
                vm.toString(address(vaultFactory)),
                "\n",
                "LEADER_TERMINAL=",
                vm.toString(address(leaderTerminal)),
                "\n",
                "LEADERBOARD=",
                vm.toString(address(leaderboard)),
                "\n",
                "PERFORMANCE_TRACKER=",
                vm.toString(address(performanceTracker)),
                "\n",
                "UNISWAP_V3_ADAPTER=",
                vm.toString(address(uniswapAdapter)),
                "\n"
            )
        );
        ...
        try vm.writeFile("deployments/base-sepolia.txt", deploymentInfo) {
            ...
            console.log(
                "Warning: could not write to deployments/base-sepolia.txt (fs permissions). Below are the addresses:"
            );
