...
contract TestAllContracts is Script {
    ...
    function testLeaderboard() internal {
        ...
        TribeLeaderboard.LeaderProfile[] memory topByApr = leaderboard.getTopLeaders(
            TribeLeaderboard.RankingCriteria.NET_APR,
            5
        );
        console.log("Top Leaders by APR:", topByApr.length);
        
        // Test: Get top leaders by TVL
        TribeLeaderboard.LeaderProfile[] memory topByTvl = leaderboard.getTopLeaders(
            TribeLeaderboard.RankingCriteria.TOTAL_TVL,
            5
        );
        console.log("Top Leaders by TVL:", topByTvl.length);
        
        // Test: Filter by risk level
        TribeLeaderboard.LeaderProfile[] memory lowRisk = leaderboard.filterByRiskLevel(
            TribeLeaderboard.RiskLevel.LOW
        );
