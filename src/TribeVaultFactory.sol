...
contract TribeVaultFactory is Ownable, ReentrancyGuard {
    ...
    // Events
    event VaultCreated(address indexed follower, address indexed leader, address vault, uint16 performanceFee);
    ...
    function createVault(address leader) external nonReentrant returns (address vault) {
        ...
        // Deploy new vault
        TribeCopyVault newVault = new TribeCopyVault(msg.sender, leader, Terminal, leaderData.performanceFeePercent);
