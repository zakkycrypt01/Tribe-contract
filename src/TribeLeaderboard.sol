...
contract TribeLeaderboard is Ownable {
    ...
    constructor(address _leaderRegistry, address _performanceTracker, address _vaultFactory) Ownable(msg.sender) {
    ...
    function getLeaderProfile(address leader) public view returns (LeaderProfile memory) {
        ...
        TribePerformanceTracker.PerformanceMetrics memory metrics = PERFORMANCE_TRACKER.getLeaderMetrics(leader);
     ...
     */
    function getTopLeaders(RankingCriteria criteria, uint256 limit) external view returns (LeaderProfile[] memory) {
        address[] memory allLeaders = LEADER_REGISTRY.getAllLeaders();
     ...
     */
    function filterByTokenPair(address, /* token0 */ address /* token1 */ )
        external
        view
        returns (LeaderProfile[] memory)
    {
        address[] memory allLeaders = LEADER_REGISTRY.getAllLeaders();
     ...
     */
    function getLeaderStats(address leader)
        external
        view
        returns (
            uint256 apr30Day,
     ...
     */
    function _sortLeaders(LeaderProfile[] memory profiles, RankingCriteria criteria) internal pure {
            ...
            for (uint256 j = 0; j < n - i - 1; j++) {
                ...
                if (criteria == RankingCriteria.NET_APR) {
                    ...
                    uint256 rarJ = profiles[j].riskScore > 0
                        ? (profiles[j].netApr * 100) / profiles[j].riskScore
                        : profiles[j].netApr;
                    uint256 rarJPlus1 = profiles[j + 1].riskScore > 0
                        ? (profiles[j + 1].netApr * 100) / profiles[j + 1].riskScore
                        : profiles[j + 1].netApr;
