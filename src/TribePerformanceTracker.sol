...
contract TribePerformanceTracker is Ownable {
    ...
    constructor(address _vaultFactory, address _leaderRegistry) Ownable(msg.sender) {
     ...
     */
    function recordSnapshot(address entity, uint256 totalValue, uint256 feesCollected, bool isVault) external {
        // In production, restrict to authorized contracts

        Snapshot memory snapshot =
            Snapshot({timestamp: block.timestamp, totalValue: totalValue, feesCollected: feesCollected});
     ...
     */
    function compareVsHodl(address vault, address token) external view returns (int256 difference) {
