...
contract TribeLeaderRegistry is Ownable, ReentrancyGuard {
     ...
     */
    function submitHistoricalPositions(
        HistoricalPosition[] calldata positions
    ) external {
        ...
        require(
            profitabilityPercent >= MIN_PROFITABILITY_PERCENT,
            "Does not meet 80% profitability requirement"
        );
    }
    ...
    function registerAsLeader(
        ...
        require(!isRegisteredLeader[msg.sender], "Already registered");
        require(
            leaderHistory[msg.sender].length >= MIN_HISTORICAL_POSITIONS,
            "Must submit historical positions first"
        );
